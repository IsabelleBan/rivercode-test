...
function CategoryCard({ cat, alwaysActive = false }: { cat: Category; alwaysActive?: boolean }) {
  return (
    <div
      className={`relative cursor-pointer group transition-all duration-300 ease-in-out bg-gray-50
...
export default function CategoryComponent() {
  return (
    <>
      <div className="md:hidden w-full pt-6 px-6">
        <Swiper modules={[FreeMode]} freeMode slidesPerView={1.5} spaceBetween={12} grabCursor>
          {categories.map((cat) => (
